...
int main()
{
    ...
    {
        cin>>a;
        pp[i+j+1][N-1+j-i+1]=a;
        //cout<<i+j+1<<":"<<N-1+j-i+1<<endl;
    }
    ...
    for(int j=0;j<2*N-1;j++)
    {
        ...
        {
            pp[i+1][j+1]+=pp[i][j+1];
            //cout<<pp[i+1][j+1]<<endl;
        }
    }
    long long ans=0;
    K=min(K,(long long)N-1);
    for(int i=0;i<2*N-(2*K+1);i++)
        for(int j=0;j<2*N-(2*K+1);j++)
        {
            //cout<<"hell"<<endl;
            //cout<<pp[i+K+1][j+K+1]<<"="<<pp[i+K+1][j]<<"="<<pp[i][j+K+1]<<"="<<pp[i][j]<<endl;
            ans=max(ans,(long long)pp[i+K*2+1][j+K*2+1]-pp[i+K*2+1][j]-pp[i][j+K*2+1]+pp[i][j]);
        }
    cout<<ans;
 }