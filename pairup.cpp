...
int main()
{
    ...
    while(fast>=slow)
    {
        if(fast==slow)
        {
            ans=max(ans,g[slow].first*2);
            break;
        }
        else
        {
            //cout<<slow<<"-"<<fast<<endl;
            ans=max(ans,g[fast].first+g[slow].first);
            if(g[fast].second<g[slow].second)
            {
                g[slow].second-=g[fast].second;
                
                fast-=1;
            }
            else if(g[fast].second>g[slow].second)
            {
                g[fast].second-=g[slow].second;
                slow+=1;
            }
            else
            {
