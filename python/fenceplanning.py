...
def union(xx,yy): 
    ...
    if rank[x]>rank[y]:
        ...
        rank[x]+=1
for i in range(k):
    x,y=tuple(map(int,fin.readline().strip().split()))
    x-=1
    y-=1
    if find(x)!=find(y):
        union(x,y)
...
for i in range(n):
    ...
    if ii not in d:
        ...
        d[ii]=[l[i][0],l[i][1],l[i][0],l[i][1]]
    else:
        d[ii][0]=min(d[ii][0],l[i][0])
        d[ii][1]=min(d[ii][1],l[i][1])
        d[ii][2]=max(d[ii][2],l[i][0])
        d[ii][3]=max(d[ii][3],l[i][1])
#print(d,parent)
for k in d:
    ans=min(ans,d[k][2]-d[k][0]+d[k][3]-d[k][1])
ans*=2	
#print(ans)
#smax=0
#print(datetime.now())
